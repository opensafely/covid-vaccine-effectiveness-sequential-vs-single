...
# get brand-specific parameters study dates and parameters
dates <- map(study_dates[[brand]], as.Date)
...
data_alltreated <- read_rds(
  ghere("output", "sequential", brand, "treated", "data_treatedeligible.rds")
  ) %>%
  mutate(treated=1L)
...
if(matching_round>1){
  ...
  data_matchstatusprevious <- read_rds(
    ghere("output", "sequential", brand, "matchround{matching_round-1L}", "actual", "data_matchstatus_allrounds.rds")
    ) %>%
...
local({
  ...
  # trial index start at one, not zero. i.e., study start date is "day 1" 
  # (but the _time_ at the start of study start date is zero)
  start_trial_time <- 0
  ...
  for(trial in trials){
    ...
    obj_matchit_i <-
      safely_matchit(
        ...
        # these two options don't really do anything because we only want exact + caliper matching:
        method = "nearest", distance = "glm", 
        replace = FALSE,
        estimand = "ATT",
        exact = exact_variables,
        caliper = caliper_variables, std.caliper=FALSE,
        # data is sorted on (effectively random) patient ID
        m.order = "data", 
        #verbose = TRUE,
    ...
    data_matched_i <-
      data_matchstatus_i %>%
      # remove unmatched people. equivalent to weight != 0
      ...
      group_by(match_id) %>%
      mutate(
        # this only works because of the group_by statement above! do not remove group_by statement!
        controlistreated_date = vax1_date[treated==0], 
      ) %>%
      ungroup()
  ...
  data_matchstatus <<-
    ...
    mutate(
      # matched=1 if matched, 0 if unmatched
      matched = replace_na(matched, 0L), 
      # control=1 if matched control, 0 if matched treated, NA if unmatched treated
...
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # 
# save matched datasets and print matching reports ----

write_rds(
  data_matchstatus,
  ghere("output", "sequential", brand, "matchround{matching_round}", "potential", "data_potential_matchstatus.rds"), 
  compress="gz"
  )
...
data_matchstatus %>% 
  ...
  write_csv(
    ghere("output", "sequential", brand, "matchround{matching_round}", "potential", glue("potential_matchedcontrols.csv.gz"))
    )
